...
// aggregate type includes both struct and array
// here we only want to get the aggregate struct
template <typename T>
inline constexpr bool IsAggregateStruct =
    std::is_aggregate_v<RemoveCVRef<T>> && !IsFixedArray<T>;
